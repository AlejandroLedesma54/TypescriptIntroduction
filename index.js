...
let nombres = ["Alejandro", "Manuela", "Santiago", "Paulina", "Jose", "Suaza"];
...
// 1
const numbersToSum = [1, 5, 6, 9, 10];
const total = numbersToSum.reduce((a, v) => a + v, 0);
console.log(total);
// 2
...
// Funciones
// Tarea 1
const newArray = [5, 85, 1];
const maxValue2 = function (value) {
    return Math.max(...value);
};
console.log(maxValue2(newArray));
// Tarea 2
const verify = function validateParameters(first, second, ...rest) {
...
const validateNumber = (number) => {
    const validate = number === 0 ? "Number is 0" : number > 0 ? "Number is positive" : "Number is negative";
    return validate;
};
console.log(validateNumber(-1));
const arrayNum = [2, 5, 10];
function maxNum(arrayNum) {
    return arrayNum.sort((a, b) => b - a)[0];
}
console.log(maxNum(arrayNum));
function restParameters(...args) {
    const firstParameterType = typeof args[0];
    const secondParameterType = typeof args[1];
